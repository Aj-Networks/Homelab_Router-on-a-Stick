...
| Date | Change |
|---|---|
| **2026-07-31** | Six weeks of IDS false positives resolved at root cause. Two earlier fixes had treated symptoms. The answer came from measuring the alert distribution instead of chasing signatures: one ruleset, `stream-events.rules`, accounted for ~99.5% of roughly 980,000 recorded alerts across 22 signature IDs. It flags TCP conditions that are abnormal on a clean LAN and completely normal on a VPN-tunneled one, so it fired hardest against the busiest destinations and filled the block table with content delivery networks rather than attackers. Disabled on all six interfaces. A parallel audit found that no threat rulesets had ever been enabled, meaning the IDS had been blocking legitimate traffic on the basis of packet timing while detecting nothing real; nine known-bad-indicator categories are now loaded on WAN in detect-only mode as a staged rollout. |
| **2026-07-31** | Three latent faults found on WireGuard interfaces rebuilt the day before, none of which produced a symptom pointing at itself. A gateway was monitoring its own interface address, so failover could never fire while the dashboard read healthy. A replacement tunnel was left at the default MTU instead of 1420, producing a PMTU black hole and throughput swinging between 240 and 900 Mbps. One of two DNS forwarders had no route and had never resolved, leaving a single live resolver behind a config that claimed two. Tunnel build checklist added to [`configs/vpn-failover.md`](configs/vpn-failover.md) so replacement interfaces stop inheriting nothing. |
...
<details>
<summary>See more (10 older entries)</summary>
